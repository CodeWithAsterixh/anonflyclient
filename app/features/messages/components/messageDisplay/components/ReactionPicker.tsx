...
import { defaultEmojis, type Emoji } from "~/shared/assets/emojis";
...
export const ReactionPicker: React.FC<ReactionPickerProps> = ({
  ...
  <div className="bg-background/95 backdrop-blur-md border border-border rounded-full px-2 py-1.5 shadow-2xl flex items-center gap-1">
    {defaultEmojis.map((emoji) => {
      ...
      return (
        <button
          ...
          className={`w-10 h-10 flex items-center justify-center rounded-full text-xl transition-all hover:scale-125 active:scale-90 ${isSelected ? "bg-primary/20 ring-1 ring-primary" : "hover:bg-white/5"
            }`}
        >
          {emoji.value}
        </button>
    ...
    >
      <Plus
        className={`w-5 h-5 transition-transform duration-300 ${showAllEmojis ? "rotate-45" : ""
          }`}
      />
    </button>
  </div>
