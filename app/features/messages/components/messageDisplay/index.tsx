...
export const MessageDisplay: React.FC<MessageDisplayProps> = ({
  ...
  useEffect(() => {
    if (showReactions) {
      ...
      document.body.style.paddingRight = `${globalThis.window.innerWidth - document.documentElement.clientWidth
        }px`;
    } else {
  ...
  return (
    <>
      <article
        ...
        className={`flex items-end gap-2 mb-4 relative transition-all duration-500 ${isCurrentUser ? "flex-row-reverse" : "flex-row"
      ...
      >
        <div
          className={`absolute top-1/2 -translate-y-1/2 transition-opacity duration-200 ${Math.abs(swipeOffset) > 20 ? "opacity-100" : "opacity-0"
