...
import { usePWA, colorSchemes, type ColorScheme } from '~/shared/hooks';
        ...
        <section className="space-y-4">
          ...
          <div className="bg-background rounded-3xl border border-border divide-y divide-border overflow-hidden">
            {identities.map((id) => (
              <div
                key={id.aid}
                className={`p-4 flex items-center justify-between gap-3 group transition-colors ${id.aid === user?.userId
                  ? 'bg-primary/10'
                  : 'hover:bg-muted-bg'
                  }`}
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold shrink-0 ${id.aid === user?.userId ? 'bg-primary shadow-md shadow-primary/20' : 'bg-muted'
                    }`}>
                ...
                {(Object.keys(colorSchemes) as ColorScheme[]).map((schemeKey) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative flex items-center justify-center w-10 h-10 rounded-full transition-all hover:scale-110 active:scale-95 ${isActive ? 'ring-2 ring-offset-2 ring-primary ring-offset-background' : ''
                        }`}
