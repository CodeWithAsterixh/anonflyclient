...
const MessageDisplay: React.FC<MessageDisplayProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Also reset isBubbleFocused when reactions close
  ...
  return (
    <>
      <div
        ...
        className={`isolate flex items-end gap-2 overflow-x-0 mb-4 relative transition-all duration-500 ${
      ...
      >
        ...
        <div
          className={`flex-shrink-0 mb-1 transition-opacity duration-300 ${
            showReactions ? "opacity-0" : "opacity-100"
          }`}
        >
          <img
            src={avatarUrl}
        ...
        <div
          ...
          className={`min-w-[2rem] w-fit line-break max-w-[70%] md:max-w-[50%] px-2 py-2 rounded-2xl shadow-sm relative transition-all duration-200 ${
            isCurrentUser ? "rounded-br-none" : "rounded-bl-none"
          } ${
            showReactions || isBubbleFocused
          ...
          {message.replyTo && (
            ...
            >
              <p className="font-bold opacity-70 truncate">
                {isReplyToMe
                  ? "You"
                  : message.replyTo.senderUsername || "Anonymous"}
              </p>
              <p className="opacity-60 line-clamp-2 italic">
          ...
          {message.reactions && message.reactions.length > 0 && (
            <div className="absolute -bottom-3 flex flex-wrap gap-1 z-20 px-1">
              {message.reactions
                .reduce(
                  (
                    acc: { emoji: string; count: number; users: string[] }[],
                    curr
                  ) => {
                    const existing = acc.find(
                      (a) => a.emoji === curr.emojiValue
                    );
                    if (existing) {
                      existing.count++;
      ...
      {showReactions && portalRoot && createPortal(
        <div
          className={
            `pointer-events-auto absolute top-0 h-full inset-0 z-[100] bg-black/10 backdrop-blur-2xl transition-all duration-300 flex items-center p-4 ${isCurrentUser?"justify-end":"justify-start"}`
          }
          ...
          >
            ...
            >
              ...
              <div
                ...
                className={`min-w-[5rem] w-fit line-break max-w-[70%] md:max-w-[50%] px-2 py-2 rounded-2xl shadow-sm relative transition-all duration-200 ${
                  isCurrentUser ? "rounded-br-none" : "rounded-bl-none"
                } shadow-2xl ring-4 ring-black/5`}
              >
                {/* Reply Preview in Bubble */}
            ...
            {/* Reaction and Action Menus */}
            <div
              ref={reactionsRef}
              className={`flex flex-col gap-2 z-[102] ${
                isCurrentUser ? "items-end" : "items-start"
              }`}
            >
              <div className="bg-white/95 backdrop-blur-md border border-gray-200 rounded-full p-1.5 shadow-2xl flex items-center gap-1 animate-in zoom-in-50 duration-200">
                ...
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center hover:bg-gray-100 rounded-full transition-all ${
                    showAllEmojis ? "bg-blue-50 text-blue-600" : "text-gray-500"
                  }`}
                >
