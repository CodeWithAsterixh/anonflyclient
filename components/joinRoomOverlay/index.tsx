...
const JoinRoomOverlay: React.FC<JoinRoomOverlayProps> = ({
  ...
  return (
    <div className="absolute inset-0 bg-background/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-background p-6 rounded-2xl shadow-xl text-center border border-border max-w-sm w-full mx-4">
        <p className="text-lg font-semibold text-foreground">{message}</p>
        {replaceLoading || (
          <div className="mt-4 animate-spin rounded-full h-12 w-12 border-4 border-border border-t-primary mx-auto"></div>
        )}
      </div>
    </div>
  );
